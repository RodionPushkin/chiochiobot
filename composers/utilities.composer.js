...
composer.command('generate',async (ctx)=>{
    try {
        if(ctx.message.chat?.username && await access.check(ctx.message.chat.username) && await access.checkChatPrivate(ctx)){
            ...
            ctx.replyWithHTML(config.text.dontHavePermission)
        }
    } catch (e) {
        console.log(e)
    }
                        ...
                        if(counter < res.fileList[0].files.length){
                            ...
                            setTimeout(async ()=>{
                                await notification.send((item)=>{
                                    ctx.telegram.sendDocument(Number(item.tgid),
                                        { source: `${__dirname.replace('composers','')}tmp\\${item.username}.xlsx` },
                                        {caption: config.text.notification.replace('%%user%%',item.username).replace('%%month%%',new Date().toLocaleString('ru', { month: 'long' }))})
                                }).then(()=>{
        ...
        if(ctx.message.chat.id == "1299761386"){
            await statistics.getMessageActivity().then(async res=>{
                ...
                items.map(async item =>{
                    return await ctx.telegram.getChat(Number(item.tgid)).then(chat=>{
                        text+=`\n${++counter}. ${chat.title}`
                        return item
                    })
                })
                ...
                setTimeout(async ()=>{
                    if(text = "Чаты без активности за последние 24 часа, списком:"){
                        text = "Во всех чатах была активность за последние 24 часа"
                    }
                    await statistics.sendMessageTo(ctx,text)
                },5000)
            })
        }
    }catch (err) {
    ...
    try {
        if(ctx.message.text == 'мой id'){
            ...
            if(await db.query(`SELECT * FROM "Chat" WHERE "tgid" = '${ctx.message.chat.id}'`).then(res => {return res.rowCount != 0})){
                ...
                await db.query(`INSERT INTO "Message" (idchat,text,time) VALUES ('${ctx.message.chat.id}','${text}','${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()} ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}')`).then(res=>{
                })
            }
            //отключить
            else{
                db.query(`INSERT INTO "Chat" (username, code, tgid) VALUES ('${ctx.message.chat.title}', '${md5(ctx.message.chat.title)}', '${ctx.message.chat.id}')`).then(()=>{
                    console.log("чат добавлен")
                    // statistics.sendMessageTo(ctx,`чат добавлен`)
                })
            }
        }
    } catch (err){
